...
```
docs/
├── analysis/                          # Working directory (during development)
│   ├── project-overview.md
│   ├── module-inventory.md
│   └── risk-assessment.md
├── plan/                              # Working directory (during development)
│   ├── task-breakdown.md
│   ├── dependency-graph.md
│   └── milestones.md
├── progress/                          # Working directory (during development)
│   ├── MASTER.md
│   ├── phase-1-<short-name>.md
│   ├── phase-2-<short-name>.md
│   └── ...
└── archives/                          # Permanent (after Phase 6)
...
│   └── ...
└── skill/
    └── SKILL.md              # The project-level sub-SKILL, moved here after completion
```
...
When generating a task-specific sub-SKILL in Phase 4, delegate to the platform's native skill-creator. The sub-SKILL is always installed at **project level** to keep it co-located with the project it serves. Provide it with the following information:
...
- **Content outline**:
  ...
  6. Archive trigger (when all tasks done, initiate archiving)
